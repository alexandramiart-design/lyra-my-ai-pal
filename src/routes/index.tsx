...
export function Page() {
  ...
  // Owner defaults are only fallbacks: any saved value always wins,
  // otherwise settings changes (theme, avatar, name…) would be ignored.
  ...
  return (
    <Chat
      ...
      isOwner={isOwner}
      onProfileUpdated={(p) => setProfile((prev) => ({ ...(prev ?? {}), ...p }) as ServerProfile)}
    />
  );
}
  ...
  return (
    <Shell>
      <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
        <div className="mb-6 h-28 w-28 overflow-hidden rounded-full bg-white/20 backdrop-blur-md shadow-2xl ring-1 ring-white/40">
        ...
        {sending && (
          <div className="flex items-end gap-2 pl-1 justify-start">
            ...
            {searchQuery !== null ? (
              ...
              <div className="bg-white/20 px-4 py-2 text-xs text-white/80 backdrop-blur ring-1 ring-white/30 rounded-3xl rounded-bl-md">
                Lyra écrit…
              </div>
            )}
          </div>
...
function ImageWithDownload({ src }: { src: string }) {
  const [open, setOpen] = useState(false);
  const [note, setNote] = useState<string | null>(null);

  async function pick(format: ImageFormat) {
    setOpen(false);
    try {
      setNote(await downloadImage(src, format));
    } catch {
      setNote("Téléchargement impossible");
    }
  ...
  return (
    <div className="relative mb-2">
      <img src={src} alt="" className="max-h-56 rounded-2xl object-cover" />
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-label="Télécharger l'image"
        className="absolute bottom-2 right-2 inline-flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white backdrop-blur transition hover:bg-black/70"
      >
        <Download className="h-4 w-4" />
      </button>
      {open && (
        <div className="absolute bottom-11 right-2 z-10 overflow-hidden rounded-xl bg-white text-gray-900 shadow-xl">
          {(["png", "jpeg", "webp"] as ImageFormat[]).map((f) => (
            <button
              key={f}
              type="button"
              onClick={() => pick(f)}
              className="block w-full px-4 py-2 text-left text-xs font-medium hover:bg-gray-100"
            >
              {FORMAT_LABELS[f]}
            </button>
          ))}
      ...
      {note && (
        <div className="absolute inset-x-2 bottom-2 rounded-lg bg-black/70 px-2 py-1 text-center text-[11px] text-white">
          {note}
        </div>
      )}
    </div>
      ...
      <div className={"group max-w-[75%] rounded-3xl px-4 py-2.5 text-sm shadow-md " +
        ...
        {m.images?.map((src, i) =>
          isUser ? (
            ...
            <ImageWithDownload key={i} src={src} />
          ),
        )}
